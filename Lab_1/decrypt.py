...
for i in range(max_len):
    ...
    for ct in cts:
        if i < len(ct):
            ...
            # Also test if XORing with this byte would produce a lowercase letter (a-z)
            # Then the corresponding plaintext in other ciphertexts would be uppercase (A-Z)
            for c in range(ord('a'), ord('z')+1):
                possible_key = ct_byte ^ c
                counts[possible_key] += 0.5  # Lower weight than spaces
